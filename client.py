...
def load_or_generate_keys_sync():
    ...
    if os.path.exists(PRIVATE_KEY_FILE):
        print(f"Loading quantum-resistant private key from {PRIVATE_KEY_FILE}...")
        try:
            with open(PRIVATE_KEY_FILE, 'r') as f:
                key_data = f.read()
            g_private_key = security.load_private_key(key_data)
            # For Kyber, we need to regenerate the public key from private key
            # Store both keys together
            parts = key_data.split('\n---PUBLIC---\n')
            if len(parts) == 2:
                g_public_key = security.load_public_key(parts[1])
            else:
                # Old format, regenerate
                print("Old key format detected, regenerating quantum-resistant keys...")
                os.remove(PRIVATE_KEY_FILE)
                load_or_generate_keys_sync()
                return
        except Exception as e:
            print(f"Error loading key, generating new quantum-resistant keys: {e}")
            if os.path.exists(PRIVATE_KEY_FILE):
                os.remove(PRIVATE_KEY_FILE)
            load_or_generate_keys_sync()
            return
    else:
        print("No private key found, generating quantum-resistant Kyber-1024 key pair...")
        g_private_key, g_public_key = security.generate_key_pair()
        priv_data = security.serialize_private_key(g_private_key)
        pub_data = security.serialize_public_key(g_public_key)
        # Store both keys in one file
        combined_data = priv_data + '\n---PUBLIC---\n' + pub_data
        try:
            with open(PRIVATE_KEY_FILE, 'w') as f:
                f.write(combined_data)
            print(f"Saved new quantum-resistant key pair to {PRIVATE_KEY_FILE}")
        except Exception as e:
    ...
    def handle_encrypted_chat(self, data: dict):
        ...
        try:
            ...
            if target == "Everyone":
                ...
                crypto_type = "KYBER-1024+AES-GCM" # Quantum-resistant
